...
目标：让 Pico 4 的 2D 悬浮窗（运行在独立虚拟 display NS_APP 上的应用，如浏览器、手游）支持**多点触控**，从而可以用左右手柄实现类似手机的双手指操作（如 MOBA 游戏的移动 + 放技能）。
...
- 请勿照做后拿它去刷写设备；本仓库不承担任何因刷机/改造导致的设备问题。
...
> 
> 由于这个"路由"发生在原生代码流程内部、且当前设备缺少可用的通用 inline-hook 库（无 Dobby，仅有 LSPlant/`liboat_hook`），**所有外部注入手段（标准 input、sendevent、自建 uinput、Xposed 替换设备、idc 路由）都无法让触摸进入 2D 窗口**。
...
├── README.md                 # 本文件
├── docs/
│   ├── 01-背景与目标.md          # 为什么做、2D 悬浮窗有多点触控需求
...
| 谁承载 2D 触摸 | XRShell (`com.picoxr.xrshell`) 的 `libxrshell.so: create_device()` 创建 uinput 设备 `virtual_input_device`（bin 名，单点 ABS_X/Y，max=100000） |
...
3. **Xposed hook 替换 XRShell 设备**（已修到 isExternal=false、Sources=0x1703 与原设备完全一致）→ viewport 仍固定主屏，不跟随 2D（路由在原生流程内）
